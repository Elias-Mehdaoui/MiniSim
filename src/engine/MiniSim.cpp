...
namespace minisim {
    ...
    ALWAYS_INLINE void count_sort(Particles& particles) {
        constexpr uint32_t max_val = N_CELLS;
        uint32_t *count = particles.cells_head; // using cells_head as tmp since we will rewrite it just after

        for (uint32_t i = 0; i < N_PARTICLES; ++i) {
            count[particles.datas[i].cell_id]++;
        }
        ...
        for (int32_t i = N_PARTICLES - 1; i >= 0; --i) {
            uint32_t idx = --count[particles.datas[i].cell_id];

            particles.tmp_datas[idx] = particles.datas[i];
            particles.tmp_vel[idx] = particles.vel[i];
            particles.tmp_pos[idx] = particles.pos[i];
        }

        uint32_t j = 0;
        while (j < N_PARTICLES) {
            uint16_t curr_cell_id = particles.datas[j].cell_id;
            particles.cells_head[curr_cell_id] = j;
            while (j < N_PARTICLES && particles.datas[j].cell_id == curr_cell_id) {
                particles.datas[j] = particles.tmp_datas[j];
                particles.pos[j] = particles.tmp_pos[j];
                particles.vel[j] = particles.tmp_vel[j];
                j++;
            }
        }
    ...
    void MiniSim::update() {
        ...
        for (size_t i = 0; i < N_CELLS; ++i) {
            ...
            for (size_t j = 0; j < size; ++j) {
                ...
                curr_cell++;
                while (particles_.cells_head[curr_cell] == 0) {
                    curr_cell++;
                }
            }
    ...
    size_t MiniSim::join_cells(uint32_t cell_id) {
        const uint32_t col = cell_id % CELL_BY_ROW;
        const uint32_t row = cell_id / CELL_BY_ROW;
        size_t size = 0;

        auto copy = [&](uint32_t offset) {
            uint32_t curr_cell = cell_id + offset;
            uint32_t idx = particles_.cells_head[curr_cell];
            if (idx == 0)
                return ;
            while (particles_.datas[idx].cell_id == curr_cell) {
